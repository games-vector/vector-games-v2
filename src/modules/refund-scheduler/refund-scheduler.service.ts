  ...
  private async refundBets(bets: Bet[]): Promise<void> {
    ...
    const refundTransactions = await Promise.all(bets.map(async (bet) => {
      ...
      try {
        ...
        gamePayloads = {
          platform: DEFAULTS.CHICKEN_ROAD.GAME_PAYLOADS.PLATFORM,
          gameType: DEFAULTS.CHICKEN_ROAD.GAME_PAYLOADS.GAME_TYPE,
          gameCode: bet.gameCode || DEFAULTS.CHICKEN_ROAD.GAME_CODE,
          gameName: DEFAULTS.CHICKEN_ROAD.GAME_NAME,
          settleType: DEFAULTS.CHICKEN_ROAD.GAME_PAYLOADS.SETTLE_TYPE,
        };
      }
