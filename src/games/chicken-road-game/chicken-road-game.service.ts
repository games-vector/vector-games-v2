...
const GAME_CONSTANTS = {
  TOTAL_COLUMNS: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.totalColumns,
  HAZARD_REFRESH_MS: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.hazardRefreshMs,
  DECIMAL_PLACES: DEFAULTS.GAMES.CHICKEN_ROAD.GAME.DECIMAL_PLACES,
  INITIAL_STEP: DEFAULTS.GAMES.CHICKEN_ROAD.GAME.INITIAL_STEP,
  PLATFORM_NAME: DEFAULTS.GAMES.CHICKEN_ROAD.GAME.PLATFORM_NAME,
  GAME_TYPE: DEFAULTS.GAMES.CHICKEN_ROAD.GAME.GAME_TYPE,
  GAME_CODE: DEFAULTS.GAMES.CHICKEN_ROAD.GAME_CODE,
  GAME_NAME: DEFAULTS.GAMES.CHICKEN_ROAD.GAME_NAME,
} as const;

const HAZARD_CONFIG = {
  [Difficulty.EASY]: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.hazards.EASY,
  [Difficulty.MEDIUM]: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.hazards.MEDIUM,
  [Difficulty.HARD]: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.hazards.HARD,
  [Difficulty.DAREDEVIL]: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.hazards.DAREDEVIL,
} as const;

// Combine platform and game-specific error messages
const ERROR_MESSAGES = {
  ...DEFAULTS.PLATFORM.ERROR_MESSAGES,
  ...DEFAULTS.GAMES.CHICKEN_ROAD.ERROR_MESSAGES,
};

@Injectable()
export class ChickenRoadGameService {
  private logger = new Logger(ChickenRoadGameService.name);

  private readonly DEFAULT_CONFIG = {
    totalColumns: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.totalColumns,
    hazardRefreshMs: DEFAULTS.GAMES.CHICKEN_ROAD.hazardConfig.hazardRefreshMs,
  ...
  async performBetFlow(
    ...
    // Normalize difficulty to uppercase for lookup (coefficients are stored as EASY, MEDIUM, HARD, DAREDEVIL)
    const difficultyNormalized = difficultyUC.toUpperCase();
    const coeffArray: string[] = coefficients[difficultyNormalized] || [];

    if (!coeffArray || coeffArray.length === 0) {
      this.logger.error(
        `No coefficients found for difficulty ${difficultyNormalized} (original: ${difficultyUC}) user=${userId}. Available keys: ${Object.keys(coefficients).join(', ')}`,
      );
    ...
    const resp: BetStepResponse = {
      isFinished: false,
      coeff: DEFAULTS.GAMES.CHICKEN_ROAD.GAME.DEFAULT_COEFF,
  ...
  async performStepFlow(
    ...
    if (endReason === 'hazard') {
      settlementAmount = DEFAULTS.GAMES.CHICKEN_ROAD.GAME.SETTLEMENT_AMOUNT_ZERO;
  ...
  private getStepCoeff(session: GameSession, stepIndex: number): number {
    if (stepIndex < 0) return DEFAULTS.GAMES.CHICKEN_ROAD.GAME.DEFAULT_MULTIPLIER;
    const arr: string[] = session.coefficients || [];
    const raw = arr[stepIndex];
    const val = parseFloat(raw);
    return isFinite(val) ? val : DEFAULTS.GAMES.CHICKEN_ROAD.GAME.DEFAULT_MULTIPLIER;
  }
  ...
  async getGameConfigPayload(gameCode: string): Promise<GameConfigPayload> {
    try {
      ...
      let coefficients = this.tryParseJson(coeffRaw) || {};
      
      // If coefficients is empty or doesn't have the expected structure, use defaults
      if (!coefficients || Object.keys(coefficients).length === 0) {
        this.logger.warn(
          `[${gameCode}] No coefficients found in config. Using default coefficients from DEFAULTS.`,
        );
        // Fallback to default coefficients from config
        coefficients = DEFAULTS.GAMES.CHICKEN_ROAD.coefficients;
      } else {
        // Log available difficulty keys for debugging
        const availableKeys = Object.keys(coefficients);
        this.logger.debug(
          `[${gameCode}] Loaded coefficients with keys: ${availableKeys.join(', ')}`,
        );
      }
      ...
      return {
        ...
        coefficients,
        lastWin: {
          username: DEFAULTS.GAMES.CHICKEN_ROAD.LAST_WIN.DEFAULT_USERNAME,
          winAmount: DEFAULTS.GAMES.CHICKEN_ROAD.LAST_WIN.DEFAULT_WIN_AMOUNT,
          currency: DEFAULTS.GAMES.CHICKEN_ROAD.LAST_WIN.DEFAULT_CURRENCY,
        }
      }
    } catch (e) {
      this.logger.error(`Failed building game config payload: ${e}`);
      // Even on error, return default coefficients so the game can still work
      return {
        betConfig: {},
        coefficients: DEFAULTS.GAMES.CHICKEN_ROAD.coefficients,
        lastWin: {
          username: DEFAULTS.GAMES.CHICKEN_ROAD.LAST_WIN.FALLBACK_USERNAME,
          winAmount: DEFAULTS.GAMES.CHICKEN_ROAD.LAST_WIN.FALLBACK_WIN_AMOUNT,
          currency: DEFAULTS.GAMES.CHICKEN_ROAD.LAST_WIN.FALLBACK_CURRENCY,
        },
      }
    }
  }
  ...
  private generateFairnessData(
    ...
    const clientSeed = roundId?.substring(0, DEFAULTS.GAMES.CHICKEN_ROAD.FAIRNESS.CLIENT_SEED_LENGTH) || DEFAULTS.GAMES.CHICKEN_ROAD.FAIRNESS.LEGACY_CLIENT_SEED;
  ...
  async getMyBetsHistory(
    ...
    const lastWeek = new Date(Date.now() - DEFAULTS.GAMES.CHICKEN_ROAD.GAME.BET_HISTORY_DAYS * 24 * 60 * 60 * 1000);  

    const bets = await this.betService.listUserBetsByTimeRange(userId, lastWeek, new Date(), gameCode, DEFAULTS.GAMES.CHICKEN_ROAD.GAME.BET_HISTORY_LIMIT);
